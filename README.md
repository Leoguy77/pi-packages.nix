...
**9,420 packages** — see [`registry/registry.json`](./registry/registry.json) for the full list.
...
| Metric | Count |
| -------- | ------- |
| Total packages | 9,420 |
| Tier A (zero deps, direct unpack) | 6,288 |
| Tier B (has npm deps) | 3,132 |
| With lockfile (`buildNpmPackage`, cached) | 3,000 |
| Fallback (`stdenv.mkDerivation`, needs network) | 132 |
| Tier B with `npmDepsHash` computed | 1,867 |

The 132 fallback packages have genuinely unresolvable npm dep trees (private scoped packages, git dependencies, yanked packages on npm).
